...
if (arg.length === 0) {
  console.log(chalk.red('Falta ingresar la ruta, si tiene dudas puede escribir "mdLinks <ruta> --help"'));
  ...
  if (arg[1] === '--validate') {
    ...
    mdLinks(arg[0], { validate: true })
      .then((v) => {
        console.log(chalk.yellow.underline.bold('Total:'), chalk.yellow(`${totalLinks(v)}`));
        console.log(chalk.cyan.underline.bold('Unique:'), chalk.cyan(`${uniqueLinks(v)}`));
      })
      .catch((err) => { console.log(err); });
  } else if (arg[1] === '--help') {
    console.log(chalk.magentaBright(help));
  ...
  if ((arg[1] === '--validate' && arg[2] === '--stats') || (arg[1] === '--stats' && arg[2] === '--validate')) {
    mdLinks(arg[0], { validate: true })
      .then((v) => {
        console.log(chalk.yellow.underline.bold('Total:'), chalk.yellow(`${totalLinks(v)}`));
        console.log(chalk.cyan.underline.bold('Unique:'), chalk.cyan(`${uniqueLinks(v)}`));
        console.log(chalk.red.underline.bold('Broken:'), chalk.red(`${brokenLinks(v)}`));
      })
      .catch((err) => console.log((err)));
